...
for agent in agentList:
    ...
    #threading.start_new_thread( ChatAgent, (agent, agentList[agent], ) )
    #agent = threading.Thread(target=ChatAgent, args=(agent,agentList[agent]))
...
while True:
    ...
    #    for agent in ChatAgent.crashed:
    #        try:
    #            print('Respawning thread for ' + i + '...', end = '')
    #            #threading.start_new_thread( ChatAgent, (i, agentList[i], ) )
    #            agent = threading.Thread(target=ChatAgent, args=(agent,agentList[agent]))
    #            print('Success!')
    #        except Exception as e:
    #            print('\nError: unable to restart thread for ' + agent, e)
